...
class UndirectedGraph{
  ...
  int extract_min(std::vector<int> &key, std::vector<bool> &visited){
    int index = -1;
    int current_min = MAX_VERTEX_COST;
    
    for(int i=0; i<vertices.size(); i++){
      if(!visited.at(i) && key.at(i) < current_min){
        current_min = key.at(i);
        index = i;
      }
    }
    return index;
